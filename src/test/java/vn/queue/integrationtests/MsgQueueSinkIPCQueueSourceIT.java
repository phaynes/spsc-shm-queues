...
public class MsgQueueSinkIPCQueueSourceIT {
    ...
    public void shutdownDaemons() throws IOException {
        ...
        if (msgSink != null) msgSink.destroy();
        if (msgSource != null) msgSource.destroy();
    }
    ...
    public void publisherSubscriberIntegrationTest() throws IOException, URISyntaxException, InterruptedException {
        ...
        // Start Msg Sink
        msgSink = IPCQueueMainLauncher.launchIPCQueueMain(ramDiskQueueDir, "sink");

        assertFalse("Msg Sink not alive", msgSink.waitFor(2l, TimeUnit.SECONDS));

        // Start Msg Source
        msgSource = MsgQueueSinkLauncher.launchMsgSink(ramDiskQueueDir, "source");
