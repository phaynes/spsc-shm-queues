...
TEST (commandTests, testInstantiateFlyweights)
{
    ...
    const aeron::util::index_t BASEOFFSET = 256;
...
TEST (commandTests, testConnectionReadyFlyweight)
{
    clearBuffer();
    AtomicBuffer ab(&testBuffer[0], testBuffer.size());
    const aeron::util::index_t BASEOFFSET = 0;
    ...
    ASSERT_NO_THROW(
    {
        ...
        const aeron::util::index_t startOfSubscriberPositions = BASEOFFSET + 32;
        for (int n = 0; n < 4; n++)
        {
            ASSERT_EQ(
        ...
        const aeron::util::index_t startOfLogFileName = BASEOFFSET + 32 + (4 * sizeof(ImageBuffersReadyDefn::SubscriberPosition));
        ...
        const aeron::util::index_t startOfSourceIdentity = startOfLogFileName + 4 + (aeron::util::index_t)logFileNameData.length();
